...
use crate::{
    automation::{
        ...
        multisig::{
            script::{
                derive_multisig_recovery_key_id,
...
pub async fn make_utxo_multisig(
    ...
    for pk in &public_keys {
        match key_manager_service
            ...
            .await
        {
            Ok(ephemeral_pubkey) => ephemeral_pubkeys.push(ephemeral_pubkey),
            Err(e) => {
                println!("Failed for pk {}: {}", pk.to_hex(), e);
                return Err(CommandError::General(format!(
                    "Failed to derive ephemeral pubkey for {}: {}",
                    pk.to_hex(),
                    e
                )));
            }
    ...
    let script = TariScript::new(vec![Opcode::CheckMultiSigVerifyAggregatePubKey(
        m,
        ephemeral_pubkeys.len() as u8,
    ...
    let (tx_id, transaction) = output_service
        .clone()
        .create_send_to_self_with_output(vec![output_builder], fee_per_gram, input_selection, payment_id.clone())
        .await
...
pub async fn create_multisig_output(
    output_service: &mut OutputManagerHandle,
    args: CreateMultisigUtxoTransferLeaderArgs,
) -> Result<MultisigOutput, CommandError> {
    ...
    let selected_utxo =
        get_utxo_by_commitment_hash(&utxos, args.utxo_commitment_hash.clone()).ok_or(CommandError::General(
            format!("UTXO with commitment hash {} not found", args.utxo_commitment_hash),
        ))?;
    ...
    match EncryptedData::decrypt_data(
        ...
        Err(_) => Err(CommandError::General("Failed to decrypt output secrets".into())),
    }
}
